...
void MyGLWidget::initializeGL () 
{
    ...
    sx=float(0);
    sy=float(0);
    provafov=2*atan((patrMax.y-patrMin.y)/3);
...
void MyGLWidget::projectTransform ()
{
    //
/**
    double ifov= 2*atan((patrMax.y-patrMin.y)/0.0002);
    double fov = calcFOV(ra,ifov);
    glm::mat4 project = glm::perspective(fov,ra,0.0001,1000.0);
    */
    ...
    glm::mat4 project = glm::perspective(provafov,ra,1.5,d+1.5+(patrMax.z-patrMin.z));
